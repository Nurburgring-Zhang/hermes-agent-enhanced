#!/usr/bin/env python3
"""
零分积压评分器 — 处理cleaned_intelligence中ai_score_total IS NULL OR =0的条目。
从score_backlog_200_v2.py复用规则引擎，修正DB路径和WHERE条件。

用法：python3 scripts/score_zero_backlog.py
输出：评分结果（7-200条），含分数分布、高价值TOP10、低分BOTTOM5

DB路径：data/intelligence.db
表名：cleaned_intelligence
条件：(ai_score_total IS NULL OR ai_score_total = 0)
上限：200条

评分引擎：六维规则评分（稀缺性/影响力/技术深度/时效性/偏好度/可信度）
总分：0-100
...
def main():
    ...
    for row in rows:
        item_id = row["id"]
        scores = score_item(row["title"] or "", row["content"] or "", row["source"] or "", row["platform"] or "")

        c.execute("""UPDATE cleaned_intelligence
        ...
        results.append((item_id, scores["total"], scores["preference"], row["title"][:50]))

    conn.commit()
    conn.close()

    totals = [r[1] for r in results]
    log(f"评分处理完成: {len(results)}条")
