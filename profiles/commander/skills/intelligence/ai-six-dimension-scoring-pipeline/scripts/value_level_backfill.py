#!/usr/bin/env python3
"""
value_level_backfill.py — 基于ai_score_total批量修复cleaned_intelligence的value_level映射

用法:
    python3 scripts/value_level_backfill.py

功能:
    将cleaned_intelligence中所有条目的value_level重新映射为基于ai_score_total的正确值。
    规则:
        5 (极高价值,即时推送): ai_score_total >= 80
        4 (高价值,推荐推送): ai_score_total >= 65 AND < 80
        3 (中价值):            ai_score_total >= 50 AND < 65
        2 (一般):              ai_score_total >= 35 AND < 50
        1 (低价值):            ai_score_total >= 20 AND < 35
        0 (垃圾):              ai_score_total < 20

场景:
    - 规则引擎评分或AI评分已经完成，但value_level从未根据ai_score_total正确映射
    - 管道(hermes_intelligence_pipeline.py)只能处理value_level >= 4的数据，
      如果映射出错，大量有价值数据被埋没
    - 首次运行后，后续增量只需更新新评分条目的value_level

输出:
    - 修复前后分布对比
    - 各level的score验证（min/max/avg/count）
    - 管道可用数据统计(>=3级/>=4级/>=5级)
    - 日志: logs/value_level_backfill_YYYYMMDD.log

依赖:
    - intelligence.db 的 cleaned_intelligence 表
    - 表中必须有 ai_score_total 字段（非NULL且有值）
"""
import json
import sqlite3
...
def main():
    ...
    conn.close()
    log(f"\n✅ value_level修复完成: 共更新{total_updated}条")
