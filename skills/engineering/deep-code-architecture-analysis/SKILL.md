...
metadata:
  hermes:
    tags: [architecture-analysis, reverse-engineering, code-review, technical-research, comparison, extreme-audit]
...
**极端审核铁律（用户明确要求，不可违反）**：
1. **逐行审核，零容忍表面功夫** — 不能只说"看起来没问题"。每行都要读，每个分支都要追。
2. **审核必须实际运行测试** — 不能只看代码就下结论。必须用curl/浏览器/TestClient真实调用端点验证HTTP状态和返回内容。
3. **vendor目录也要审** — 第三方代码是本项目的一部分，逐行审完。用户不接受"vendor不是我们的代码"作为借口。
4. **修复后必须验证** — 静态审核发现bug只是第一步。修完后必须运行时验证(调用该函数/端点/方法)确认修复有效。
5. **双视角验证** — 代码审核和运行时测试是两回事。静态看起来对的代码运行时可能500错误。
6. **bug详情必须输出完整清单** — 每次审核后输出完整的"已知问题清单"（未修复的问题+原因+下一轮优先做什么）。
...
| `codebase-inspection` | Subsection — pygount-based LOC/language/comment ratio analysis | `references/pygount-codebase-inspection.md` |
...
### Codebase Inspection with pygount (LOC + language stats)

When the user asks for basic codebase metrics (LOC, language breakdown, comment ratio), use pygount. See the full guide at `references/pygount-codebase-inspection.md`.
...
Always use `--folders-to-skip` or pygount will crawl node_modules and hang. For deeper structural/architecture analysis, proceed to the 7-step workflow below instead.

### 14-Dimension Software Engineering Audit + Fix

For the user request "以真实软件工程的完整方法进行全流程分析、审核、测试" (full software engineering audit with real methodology), load `references/14-dimension-software-audit.md`. This covers all 14 dimensions with a P0→P3 priority system and execution pattern.

For the **full audit → prioritize → fix → verify** workflow (including bare except repair, dead code cleanup, TypeScript strict mode, security hardening, and regression testing), load the `full-stack-codebase-audit` engineering skill. It extends the audit pattern with a Step 4-5 execution loop and tracks real fixes from production sessions.
...
- **Lambda闭包延迟绑定** — `for ext in to_block: await context.route(f"**/*.{ext}", lambda route: route.abort())` — 所有lambda共享循环结束后的最后一个值。修复: `lambda route, e=ext: ...`
...
7. **Lambda闭包延迟绑定** — 循环中创建的lambda引用循环变量而非当前值。修复: 默认参数捕获 `lambda route, e=ext: ...`
...
### 🕳️ 陷阱7: 审核不运行测试
**现象**: 审完代码说"看起来没问题"，但实际HTTP端点返回500、函数调用报错、配置不兼容
**对策**: 审核流程必须包含真实的运行时验证步骤。每个审核结论必须伴随一个实际的终端测试命令(如curl请求、python -c调用的输出)。
**这是用户最厌恶的错误模式** — 比代码写错本身更严重。

### 🕳️ 陷阱8: 跳过vendor目录
**现象**: vendor/里的第三方代码被认为是"别人的代码不需要审"
**对策**: vendor目录是本项目的组成部分。用户要求逐行审，包括vendor。
