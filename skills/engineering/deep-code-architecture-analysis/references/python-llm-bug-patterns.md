...
## Pattern 7: Lambda Closure Late Binding

**Symptom:** All route handlers/callbacks use the last value from the loop, not each individual value.

**Root cause:** Python's lambda closures capture variables by reference, not by value. All lambdas created in a loop share the same loop variable.

**Code before fix (BUG):**
```python
for ext in to_block:
    await context.route(f"**/*.{ext}", lambda route: route.abort())
# All lambdas abort **/*.pdf instead of each specific extension
```
...
**Detection:** Search for `lambda route:` or `lambda x:` inside a `for` loop that references the loop variable.

**Similar patterns in JavaScript:**
```javascript
// BUG: all buttons alert "Button 3"
for (var i = 0; i < 3; i++) {
    button.onclick = function() { alert("Button " + i); };
}
// FIX: let scope or closure
for (let i = 0; i < 3; i++) {
    button.onclick = function() { alert("Button " + i); };
}
```
