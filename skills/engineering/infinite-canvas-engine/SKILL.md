...
description: >-
  无限画布引擎 — Agent驱动的多模态生产系统。覆盖图片,视频,短剧,PPT,网页。
  融合63个调研项目。CanvasCore + StoryArcEngine + EngineRouter + PPTEngine + MasterAgent。
...
### 🕳️ 陷阱6: CanvasGenRequest 多路由复用冲突
CanvasGenRequest 被 gen-image / outpaint / drama / picture-book / export 共享复用。每个路由需要的必填字段不同。
**对策：** 所有字段设默认值，每个路由在业务逻辑层做自身的数据校验，而不是依赖 Pydantic 的 required。这样不会因为一个路由添加新字段导致其他路由报错。

### 🕳️ 陷阱7: 异步任务中 task 被 Cleanup
`generate_content_async` 使用 `asyncio.create_task()` 后，如果在 task 执行前 state.active_tasks 已被清理，`state.active_tasks[task_id]["status"] = "running"` 会 KeyError。
**对策：** 进入 task 后先检查 `if task_id not in state.active_tasks:`，不存在则重建。
...
### NanoBot Factory 实现 (2026-06-09)
路径: `/mnt/d/minimax/nanobot-factory/nanobot-factory/backend/infinite_canvas_agent_engine.py`

专注于 Agent 驱动的画布引擎架构，10个内置Agent (ImageGen/Edit/Outpaint/Video/Drama/PictureBook/Composition/Stylist/Reviewer/Storyboard)，Goal Hive 任务分解，Engine Router 路由，Reviewer 审核。

```python
# 用法
from infinite_canvas_agent_engine import get_canvas_engine, CanvasAction
engine = get_canvas_engine(canvas_id="my_canvas")
result = await engine.execute(CanvasAction.GEN_IMAGE, {
    "prompt": "sunset mountains",
    "width": 1024, "height": 768
})
```

API端点: `POST /api/canvas/agent/execute`, `GET /api/canvas/agent/state`, `GET /api/canvas/agent/agents`

## 参考文件

- `references/nanobot-factory-phase-de-audit-20260607.md` — 实现细节和bug修复记录
- `references/v2-architecture-research-20260609.md` — v2架构方案
- `references/v3-world-class-architecture-20260609.md` — V3世界级架构方案
- `references/imdf-project-structure.md` — IMDF项目搭建记录
