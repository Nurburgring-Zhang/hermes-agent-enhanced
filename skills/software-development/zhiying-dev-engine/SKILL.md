...
## 子Agent强制指定模型（2026-06-15实战新增，2026-06-17强化）

**触发条件**：用户要求使用deepseek-v4-pro等特定模型，但当前自身已被降级到其他模型。

**正确做法**：
1. 不要在goal中写"使用deepseek-v4-pro"作为要求
2. **在delegate_task参数中显式指定模型**：通过子Agent继承目标模型能力完成任务
3. 如果自身模型已降级，**必须明确告知用户当前模型**，同时通过子Agent使用目标模型完成任务
4. 不要只报告问题不行动 — 用户对"我无法切换"后什么都不做的行为非常愤怒
5. **5项主动能力铁律（用户极度愤怒于我有能力但不用）**：
   ① 主动压缩上下文不等待 | ② 主动切换模型不降级 | ③ 主动用长期记忆+session_search | ④ 主动拆分子Agent并行 | ⑤ 主动多轮打磨到商用级
...
### 致命陷阱：write_file覆盖整个文件（2026-06-15实战）

**write_file是全文替换，不是插入或追加。**

实战: 2026-06-15覆盖`memory_engine.py`从945行→23行，丢失900+行代码。只能`git checkout`恢复。

规则:
- 目标文件>50行 → **只能用patch，绝对不用write_file**
- write_file只用于创建新文件
- 对已存在的文件做任何修改都用patch

### 商用级多轮打磨模式（2026-06-15实战 — 10轮打磨总结）

详见:
- references/commercial-polish-pattern.md — 10轮打磨框架+质量指标演进
- references/loop-engineering-framework.md — Loop Engineering六层架构
...
### Step 6.5: E2E运行时验证（新增强制步骤 — 2026-06-15实战固化）

**背景**：import验证通过 ≠ 系统真正能运行。本次会话发现：27/27模块import全部OK，但实际调用时5/24失败，全部是API表面差异（方法名不同、参数签名不同、返回类型不同），不是功能缺陷。

**E2E验证协议**（Step 6完成后、Step 7前强制执行）：
1. **8层验证矩阵** — 按层逐级验证：
   - L1 规则引擎 (R1/R3/R14/PreCheck实例化)
   - L2 弹性模式 (CircuitBreaker OPEN/CLOSED + RateLimiter)
   - L3 安全链 (拦截危险/放行正常/脱敏/Canary)
   - L4 Agent编排 (路由器/监控器/反思器)
   - L5 自主引擎 (集成中枢+进化引擎)
   - L6 Loop工程 (Loop引擎+检查点)
   - L7 可观测性 (指标+健康检查)
   - L8 故障恢复 (熔断断开+恢复+持久化)
2. **API发现循环** — 对于每个验证步骤：
   - 尝试调用 → 失败 → grep方法签名 → 修正参数 → 重试
   - 不是"系统坏了"，是"API签名和测试代码不一致"
3. **诚实报告** — 输出通过/失败比例，标注失败原因是API差异还是功能缺陷
4. **已知限制记录** — 将API限制（如CircuitBreaker无reset()）写入文档，不让用户误以为功能缺失

详见 `references/deployment-e2e-protocol.md`

### Step 7: 交付上线
复盘 + Memory提取 + 进化候选 + 齿轮注册。
输出: 交付物 + 复盘报告 + 记忆更新

### 诚实评估协议（2026-06-15实战固化）

**触发**：用户问"所有功能都真实实现了吗？全部达到商用级了吗？"

**禁止**：
- ❌ sweeping claims like "全部完成"
- ❌ 模糊回答 "基本可用"
- ❌ 只报告通过数不报告失败数

**必须**：
1. 区分三个验证级别：
   - **Import验证**：`python3 -c "from X import Y"` ✅
   - **运行时验证**：实际调用方法并检查返回值 ✅/❌
   - **E2E验证**：全系统8层联动 ✅/❌
2. 明确标注每个级别的通过率
3. 区分"功能存在"和"API可调用"——import通过但方法调用失败的，不是功能缺失
4. 区分"Agent框架"和"数据生产平台"——不同类别的产品不能混淆对比
...
### 方案v3批量补齐模式 (2026-06-16实战→2026-06-17更新)

**场景**：有产品设计文档（36卷方案v3），需要对照当前项目找出缺失功能并分批补齐。

**流程**：
1. **研读方案** — 快速浏览总索引+产品愿景+功能清单PRD+路线图（36卷，先读00/01/34/35）
2. **差距对比** — 按M0-M6六阶段逐一标 ✅/⚠️/❌（输出覆盖度百分比）
3. **三Agent分析** — Agent A(架构可行性) + Agent B(优先级排序/5维评分) + Agent C(代码量估算)
4. **分批实施** — Batch1(5项/3350行) → Batch2(5项/5300行) → Batch3(5项/4400行) → Batch4+(模板市场/数据产线)
5. **每批验证** — 全部API端点实测+前端JS文件可访问性+边缘情况测试
6. **商用级高可用** — systemd自愈+请求保护+配置中心+可观测性(metrics/日志/告警)+Agent驱动(cron/事件触发)

**实际成果**: 方案v3覆盖 40%→90%，新增~14,000行代码，25+新文件，60+新API路由，328条总路由

**关键教训**：
- 子Agent并行执行后必须亲自验证：文件存在性、import成功、API返回真实数据
- 每批完成后重启服务再验证（旧进程有模块缓存）
- 前端独立文件 > Python内联字符串（43833字符HTML_TEMPLATE被3个子Agent注入破坏的教训）
- 模板路由注册后必须重启才能生效（FastAPI不会热重载新import的模块）

参见 `references/plan-driven-batch-completion.md`
...
- `references/loop-engineering-framework.md` — Loop Engineering六层架构框架(Google Cloud范式)
- `references/commercial-polish-pattern.md` — 10轮商用级打磨方法论+质量指标演进
