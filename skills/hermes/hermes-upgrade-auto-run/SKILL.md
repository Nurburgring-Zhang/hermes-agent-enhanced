...
### 当前版本（2026-06-11）
- Hermes Agent v0.16.0 (2026.6.5)，upstream commit `a2d7f538`
- v0.16.0新增：Desktop App (Electron)、Web管理面板、/undo撤销、模糊模型搜索、简体中文翻译、CVE-2026-48710安全修复

### 升级方式
- **Shallow clone 升级**：`git fetch --depth=1 origin main` + `git reset --hard origin/main`（不要用 `git merge`，浅克隆无共同祖先会全部CONFLICT）
- 升级后必须重新注入3处核心钩子（详见 `hermes-agent-operations` skill 第七节）

### 核心注入点（v0.16.0 验证位置）
1. `agent/conversation_loop.py:666` — PRE hook（`effective_system` 构建后）
2. `run_agent.py:5123` — POST hook（`run_conversation()` 返回前）
3. `hermes_cli/__init__.py:14` — 双审启动注入（`__doc__` 结束处）

### 不受 git 影响的持久化组件
- `~/.hermes/config.yaml` — provider/fallback配置，不在git内
- `~/.hermes/scripts/` — 600+自定义脚本，不在git内
- `~/.hermes/skills/` — 180个技能，不在git内
- crontab — 66条系统crontab，独立于hermes-agent仓库
- `~/.hermes/active_memory.db` / `intelligence.db` — 数据库

### 升级后验证清单
```bash
hermes --version                                    # 版本确认
grep -c 'safe_hook_pre' agent/conversation_loop.py  # PRE hook
grep -c 'safe_hook_post' run_agent.py               # POST hook  
grep -c 'startup_dual' hermes_cli/__init__.py       # 双审启动
crontab -l | wc -l                                  # cron完整性
ls ~/.hermes/scripts/*.py | wc -l                   # scripts完整性
```
