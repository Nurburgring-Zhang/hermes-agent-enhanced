...
### B0. 升级后注入恢复流程（v0.16.0验证，2026-06-11）

每次Hermes Agent升级（`hermes update` 或 `git reset --hard`）后，3处核心注入会丢失，必须立即恢复：

**注入点位置（v0.16.0）**：
| # | 文件 | 行号 | 内容 | 验证命令 |
|---|------|------|------|----------|
| 1 | `agent/conversation_loop.py` | 667 | PRE hook — 对话前注入增强指令 | `grep -c 'safe_hook_pre_conversation' agent/conversation_loop.py` |
| 2 | `run_agent.py` | 5123-5135 | POST hook — 对话后复盘反思 | `grep -c 'safe_hook_post_conversation' run_agent.py` |
| 3 | `hermes_cli/__init__.py` | 14-17 | 启时加载双审守护 | `grep -c 'startup_dual_review' hermes_cli/__init__.py` |

**注入代码模板**（Python语法验证：`python3 -m py_compile`）：
1. PRE hook — 见 `hermes-agent-operations` skill Section 7
2. POST hook — 见 `hermes-agent-operations` skill Section 7
3. 双审启动 — `import subprocess as _sp, os as _os; _sp.run(["bash", _os.path.expanduser("~/.hermes/scripts/startup_dual_review.sh")], capture_output=True)`

**Shallow clone 陷阱**：`git merge` 导致所有文件CONFLICT，必须用 `git reset --hard origin/main`。
