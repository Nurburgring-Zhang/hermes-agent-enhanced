...
## ⚠️ CRITICAL: `--push` Flag Required

The script **defaults to DRY RUN** (no actual push). To actually send the push, you **MUST** add the `--push` flag:

```bash
# DRY RUN (preview only — NO push sent):
cd ~/.hermes && python3 scripts/hermes_v12_push.py

# ACTUAL PUSH (sends to WeChat via PushPlus):
cd ~/.hermes && python3 scripts/hermes_v12_push.py --push
```

**Cron / guardian.py integration**: Both `guardian.py push` and the crontab entry must call the script with `--push`. If they call without it, the script silently prints "DRY RUN" and exits with `exit_code=0` — no push happens, but no error either. Verify with logs: `grep "推送成功" ~/.hermes/logs/v12_push.log`.

**Verification**: After pushing, check the PushPlus history in WeChat or grep the log:
```bash
tail -3 ~/.hermes/logs/v12_push.log
# Expected: ✅ 推送成功! (HTML模板) / 已记录 N 条推送历史
```

## Verification (DRY RUN)

Run DRY RUN (no `--push` flag) to preview what would be pushed:
...
🔍 DRY RUN模式 — 不推送        ← This is NOT a failure, it's the default!
```
