...
| `references/v12-push-call-syntax.md` | [参考] `--push`参数调用语法、实测记录 |
...
### 🔴 DRY RUN模式陷阱：hermes_v12_push.py 默认不推送！

- **症状**: `python3 scripts/hermes_v12_push.py` 运行显示"推送预览"和日志，但实际不发送任何内容（打印"🔍 DRY RUN模式 — 不推送"）
- **原因**: 脚本第1030行 `if "--push" in sys.argv:` 控制实际推送操作。无 `--push` 参数时跳过 `push_wechat()` 和 `record_pushed()`
- **影响**: 
  - 所有cron任务不带 `--push` 时永远在DRY RUN
  - 手动测试时容易误以为推送成功（日志看起来一样）
  - 推送记录不会写入push_records表
- **检测**: 输出末尾看是 `🔍 DRY RUN模式` 还是 `📤 推送微信` + `✅ 推送成功!`
- **修复**: 所有cron命令和手动执行必须加 `--push`：
  ```bash
  python3 scripts/hermes_v12_push.py --push
  ```
- **教训**: 任何有DRY RUN模式的脚本，必须在技能文档中显式标注参数要求。cron配置必须包含参数。
