...
For running Codex CLI as an isolated implementation lane within a Kanban task, see `references/kanban-codex-lane.md`. The pattern: Hermes owns task lifecycle → spawns Codex in an isolated worktree → reconciles diff → runs verification → completes the Kanban task.
