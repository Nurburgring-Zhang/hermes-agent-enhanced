...
Hermes Agent Enhanced 由4大子系统构成，每个子系统独立运作并通过齿轮系统互锁：

### 子系统1: Loop Engineering 执行引擎
实现 等待→唤醒→规划→执行→验证→记录→休眠 的完整Loop生命周期。
- **loop_engine.py** (1315行) — 6阶段执行引擎 + DAG任务图 + 拓扑排序
- **loop_checkpoint.py** — 断点续传，故障恢复
- 触发模式: Cron / Webhook / File Watch / Continuous / Manual
- 并行隔离: 每个子任务独立worktree/临时目录
- 成本追踪: Token消耗统计 + 预算上限

### 子系统2: 规则强制执行引擎 (R1-R14)
14条代码级铁律，每次工具调用前后强制拦截。
- **rule_enforcer.py** (1454行) — R1反幻觉 → R14三阶段开发铁律
- 5级审计追踪 + 双AI互审 + 三阶段开发强制
- 自主边界保护: 敏感操作block/warn/alert三级响应

### 子系统3: 三省六部多Agent编排
- **ministry_abc.py** (914行) — RoleOrchestrator + WorkflowStateMachine
- 12部门130Agent编排 (agent_company_engine.py)
- 链式/并行任务调度

### 子系统4: 弹性模式 + 齿轮驱动
- **resilience_patterns.py** (446行) — 10组件: 熔断器/重试/限流/降级/审计/指标/热加载/干跑
- **gear_enforcer.py** (954行) — G0-G7齿轮系统，1分钟粒度强制互锁
...
```
              ...
              │                        │                        │
    ┌─────────▼─────────┐    ┌────────▼────────┐    ┌─────────▼─────────┐
    │   Layer 1: 执行层   │    │  Layer 2: 审核层  │    │  Layer 3: 弹性层   │
    │                    │    │                  │    │                    │
    │  ┌──────────────┐  │    │  R1 反幻觉铁律    │    │  CircuitBreaker   │
    │  │ Loop Engine  │  │    │  R2 前置三查      │    │  RetryBackoff     │
    │  │ wake→plan→   │  │    │  R3 改前备份      │    │  RateLimiter      │
    │  │ exec→verify  │  │    │  R4 交付铁律      │    │  TimeoutManager   │
    │  │ →record→sleep│  │    │  R5 深度审核      │    │  FallbackRegistry │
    │  └──────────────┘  │    │  R6 沟通风格      │    │  DecisionAudit    │
    │                    │    │  R7 自主边界      │    │  MetricsCollector │
    │  ┌──────────────┐  │    │  R8 问责          │    │  HotReloader      │
    │  │ 三省六部编排   │  │    │  R9 双AI互审      │    │  DryRunMode       │
    │  │ ministry_abc │  │    │  R10 真实实现     │    │                    │
    │  │ 130 Agent    │  │    │  R11 循环强制     │    └─────────┬─────────┘
    │  └──────────────┘  │    │  R12 智影SDLC     │              │
    │                    │    │  R13 技能强制     │              │
    └─────────┬─────────┘    │  R14 三阶段开发    │              │
    ...
    ┌─────────▼─────────┐   ┌────────▼────────┐   ┌─────────▼─────────┐
    │   记忆系统 (双轨)   │   │   审计追踪        │   │   自动CI循环       │
    │   memory_engine   │   │   audit_system   │   │   auto_ci.py      │
    │   episodic+       │   │   858行审计       │   │   lint→test→      │
    │   semantic        │   │   CloudTrail对标  │   │   coverage→sec    │
    └───────────────────┘   └─────────────────┘   └───────────────────┘
    ...
    ┌─────────▼─────────┐   ┌────────▼────────┐   ┌─────────▼─────────┐
    │  情报采集管道       │   │  上下文压缩       │   │  模型智能路由       │
    │  unified_collector│   │  force_compressor│   │  model_router     │
    │  48个采集器        │   │  三层压缩60-70%   │   │  自动切换链        │
    └───────────────────┘   └─────────────────┘   └───────────────────┘
```
